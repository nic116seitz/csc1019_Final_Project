...
# ----------------------------------------
# Import random is for debug
# import random
# String for punctuation check
# date_verify for date verification
import string
# Note: Due to upload restrictions date_verify will be included in the folder sent via email
from date_verify_Nicolas_Seitz import *
...
def edit(events_list):
    # Edit loop var
    edit_loop = 0
    # While the edit loop is still active the user will be prompted to put in the name of the edit target
    while edit_loop == 0:
        edit_target = input("\nPlease select the event you would like to edit: ")
        # for loop to compare query with event.name for each event in event list
        for event in events_list:
            # if the edit target matches event.name then the event is reassigned to the variable edit_target
            if edit_target == event.name:
                edit_target = event
                # loop condition is broken
                edit_loop += 1
            else:
                pass
        # if the loop terminates without changing the while loop condition then the error message is printed
        else:
            if edit_loop == 0:
                print("Edit target not found")
        
    # Field loop var
    field_loop = True
    while field_loop == True:
        # beginnning of try catch designed to catch non number entries
        try:
            # Field target allows the user to select what part of the event they would like to edit
            field_target = int(input("\nPlease select the field of the event you would " + 
                ...
                "6) Quit\n" +
                "Your choice:"))
            # if the field target is 1 then they can rename it by reassigning the attribute as the edit_target_rename 
            if field_target == 1:
                edit_target_rename = (input("Enter the new name you would like to put for the event(enter cancel to cancel): ")).lower()
                if edit_target_rename == "cancel":
                   field_loop == False
                else: 
                    # slicing to ensure that the name is capitalized
                    edit_target.name = edit_target_rename[0].upper() + edit_target_rename[1:]
                    field_loop = False
            # if the field target is 2 it calls the date_add function to change the date
            # also breaking the loop
            elif field_target == 2:
                date_add(edit_target)
                print("Date Changed:", edit_target.date)
                field_loop = False
            # if the field target is 3 then it calls the add_location function
            elif field_target == 3:
                add_location("Location Changed:", edit_target.location)
                field_loop = False
            # 4 calls add_tags
            elif field_target == 4:
                add_tags(edit_target)
                field_loop = False
            # 5 adds description
            elif field_target == 5:
                add_script(edit_target)
                field_loop = False
            #6 quits
            elif field_target == 6:
                print("\n Returning to menu....")
...
def listall(event_list):
    # if the event list is empty it triggers error message
    if event_list == []:
        print("You currently have no events in your calendar")
    # other wise for each event in the event_list the function calls the eventprint function
    else:
        for event in event_list:
            eventprint(event)
            
# Search function takes a query and a event list as inputs
def search(event_list, query):
    match = [] # Tracks matching events 
    query = (query.lower()).split(" ") # Turns the query into an array of words
    for event in event_list: # nested for loop for comparing each event with the query array word 
        for word in query:
            word_cap = word[0].upper() + word[1:] # capitilzed query words
            if word in event.name or word_cap in event.name or word in event.tags or word_cap in event.tags: # checks to see if query words or query word capitalized in in nam or tags
                match.append(event)
    if match == []: # if no match found program prints no match
        print("No match Found!")
    else: # Otherwise it prints all events in the match array
...
# This is a test function that generates 3 random events in order to test that the the program works
# event_inc = 1
# print("Debug at line 329")
    # for num in range(4):
...
# This compare the revenue of all events and spits out the highest revenue for all the events in the event list
...
def menu(events_list):
    ...
    while menu_loop == True:
        ...
        # The following are switch statements that allow the user to call the corresponding function depending on what number they enter
        if usr_choice == "1":
